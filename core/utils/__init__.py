...
def format_memories_for_injection(memories: list) -> str:
    ...
    for idx, mem in enumerate(memories, 1):
        try:
            ...
            # v2.5: provenance 标签 — 标明记忆来源
            source = metadata.get("source", "recall")
            provenance_tag = ""
            if source == "stream_extractor":
                provenance_tag = ", src:stream"
            elif source == "graph":
                provenance_tag = ", src:graph"
            elif source == "session_summary":
                provenance_tag = ", src:summary"
            
            entry_parts = [
                f"记忆 #{idx} / Memory #{idx} (Importance: {importance:.2f}{provenance_tag}){time_part}"
            ]
            ...
            # P1-② 逻辑自检⚠️：矛盾标注（self_check 管道写入：互检+冲突关联）
            warnings = (
                mem.get("conflict_warnings", [])
                if isinstance(mem, dict)
                else getattr(mem, "conflict_warnings", None)
            )
            if warnings:
                for w in list(warnings)[:3]:  # 最多3条防刷屏（候选限额铁律）
                    entry_parts.append(f"  ⚠️[逻辑自检] {w}")
