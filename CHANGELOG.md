...
### 新增 — 三层记忆架构 (L1/L2/L3)，借鉴 DeepTutor

- **三层记忆层级 (Tier)**: `memory_atoms` 表新增 `tier` 字段
  - **L3** (tier=3): 跨会话合成记忆 — 由合成器从多条 L2 记忆提炼生成的高层次洞察
  - **L2** (tier=2): 会话摘要记忆 — 单次会话中提取的关键事实（默认层级）
  - **L1** (tier=1): 原始消息事件 — 存储在 conversations.db 的原始对话记录
- **溯源链**: `source_ids` 字段记录记忆来源引用链 (L3 → L2 → L1)
- **L3 合成器** (`l3_synthesizer.py`): 定期扫描 L2 记忆，发现跨会话模式自动合成 L3 记忆
- **金字塔 API** (`get_memory_pyramid`): 一次性获取三层记忆数据，按重要性排序
- **记忆溯源 API** (`get_memory_trace`): 单条记忆的完整溯源链 (L3→L2→L1)

### 变更
- `memory_atoms` 表新增 `tier` 字段（默认值 2）
- `memory_atoms` 表新增 `source_ids` 字段用于溯源

## [5.4.0] - 2026-07-19

### 新增 — Context 组装显式化
- **AssemblyTrace 模型**: 记录每次上下文组装的完整链路（查询扩展 → 双路路由 → 融合排序 → 情绪加成 → 活跃窗口 → 注入 → 自省）
- **ContextTraceStore**: 独立 SQLite 存储 (`data/context_traces.db`)，不干扰主记忆库
- **情感路由 (EmotionRouter)**: 根据用户情绪自动调整召回策略，情绪匹配的记忆获得 boost
- **活跃窗口加成**: 近期记忆在召回排序中获得时间衰减加成
- **自省注入器 (SelfReflection)**: 每次注入后自动生成自然语言摘要（"本次召回了 N 条记忆，排名最高的是…"），写入 trace
- **`/lmem trace` 命令**: 查看最近一次 Context 组装的完整 trace

### 修复
- `EmotionSignal` 字段名 `keywords` → `keyword` 修复
- `command_handler.py` 语法错误修复
...
- **`/lmem summary` 命令增强**: 支持手动触发摘要生成

## [5.2.0] - 2026-07-19

### 新增 — 四大核心升级（21/21 测试通过）
- **核心记忆索引 (Core Memory Index)**: 始终在线的高优先级记忆索引，确保关键信息在每次对话中都可被访问
- **图增强召回 (Graph-Enhanced Recall)**: 在文档路和图路召回基础上，利用图谱关系增强跨记忆关联召回
- **决策追踪 (Decision Tracking)**: 记录 LLM 的关键决策路径，便于回溯"为什么这样回复"
- **注入链重排 (Injection Chain Reordering)**: 对注入到上下文的记忆片段进行智能重排序，提升相关性
